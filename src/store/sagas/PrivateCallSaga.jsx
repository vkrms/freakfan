...
import {
  ...
  audioCallChargesUpdateSuccess,
  audioCallChargesUpdateFailure,
  videoCallChargesUpdateSuccess,
  videoCallChargesUpdateFailure,
...
import {
  ...
  AUDIO_CALL_CHARGES_UPDATE_START,
  VIDEO_CALL_CHARGES_UPDATE_START,
...
function* audioCallChargeApi(action) {
  try {
    const response = yield api.postMethod(
      "audio_call_charges",
      action.data
    ...
    if (response.data.success) {
      yield put(audioCallChargesUpdateSuccess(response.data.data));
      const notificationMessage = getSuccessNotificationMessage(
        response.data.message
      );
      yield put(createNotification(notificationMessage));
    } else {
      yield put(audioCallChargesUpdateFailure(response.data));
    }
  } catch (error) {
    yield put(audioCallChargesUpdateFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}
...
function* videoCallChargeApi(action) {
  try {
    const response = yield api.postMethod(
      "video_call_charges",
      action.data
    ...
    if (response.data.success) {
      yield put(videoCallChargesUpdateSuccess(response.data.data));
      const notificationMessage = getSuccessNotificationMessage(
        response.data.message
      );
      yield put(createNotification(notificationMessage));
    } else {
      yield put(videoCallChargesUpdateFailure(response.data));
      const notificationMessage = getErrorNotificationMessage(
        response.data.error
      );
      yield put(createNotification(notificationMessage));
    ...
    yield put(videoCallChargesUpdateFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}
...
export default function* pageSaga() {
  ...
  yield all([yield takeLatest(AUDIO_CALL_CHARGES_UPDATE_START, audioCallChargeApi)]);
  yield all([yield takeLatest(VIDEO_CALL_CHARGES_UPDATE_START, videoCallChargeApi)]);
